...
export class WatchOnlyWallet extends LegacyWallet {
  ...
  allowSend() {
    // Single-address watch-only wallets can always create unsigned PSBTs.
    if (!this.isHd()) return true;

    if (!this.useWithHardwareWalletEnabled()) return false;

    if (!this._hdWalletInstance) this.init();
    return !!this._hdWalletInstance?.allowSend();
  }
  ...
  async fetchUtxo() {
    ...
    const txids = [...new Set(this._utxo.map(utxo => utxo.txid).filter(Boolean) as string[])];
    if (txids.length === 0) return;

    const txhexes = await BlueElectrum.multiGetTransactionByTxid(txids, false);
    this._utxo = this._utxo.map(utxo => {
      if (!utxo.txid) return utxo;
      return txhexes[utxo.txid] ? { ...utxo, txhex: txhexes[utxo.txid] } : utxo;
    });
  }
  ...
  combinePsbt(...args: Parameters<THDWalletForWatchOnly['combinePsbt']>) {
    ...
    const [base64one, base64two] = args;
    ...
    try {
      ...
      return final1.extractTransaction();
    }
  }
  ...
  createTransaction(...args: Parameters<THDWalletForWatchOnly['createTransaction']>) {
    ...
    // For single-address watch-only wallets (no derivation path), produce an unsigned PSBT.
    return super.createTransaction(utxos, targets, feeRate, changeAddress, sequence, true, this.getMasterFingerprint());
  }
