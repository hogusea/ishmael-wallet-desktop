...
class DeeplinkSchemaMatch {
  static hasSchema(schemaString: string): boolean {
    ...
    return (
      ...
      lowercaseString.startsWith('ishmael:') ||
  ...
  static navigationRouteFor(
    ...
    if (event.url.toLowerCase().startsWith('ishmael:bitcoin:') || event.url.toLowerCase().startsWith('ishmael:lightning:')) {
      event.url = event.url.substring('ishmael:'.length);
    } else if (event.url.toLocaleLowerCase().startsWith('ishmael://widget?action=')) {
      event.url = event.url.substring('ishmael://'.length);
    }
    ...
    if (isBothBitcoinAndLightning) {
      ...
      (async () => {
        if (urlObject.protocol === 'ishmael:' || urlObject.protocol === 'lapp:' || urlObject.protocol === 'blue:') {
  ...
  /**
   * Extracts server from a deeplink like `ishmael:setelectrumserver?server=wallet.mobick.info%3A40009%3As`
  ...
  static getServerFromSetElectrumServerAction(url: string): string | false {
    if (!url.startsWith('ishmael:setelectrumserver') && !url.startsWith('setelectrumserver')) return false;
  ...
  /**
   * Extracts url from a deeplink like `ishmael:setlndhuburl?url=https%3A%2F%2Flndhub.herokuapp.com`
   * returns FALSE if none found
   *
   * @param url {string}
   * @return {string|boolean}
  ...
  static getUrlFromSetLndhubUrlAction(url: string): string | false {
    if (!url.startsWith('ishmael:setlndhuburl') && !url.startsWith('setlndhuburl')) return false;
