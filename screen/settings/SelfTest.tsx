...
export default class SelfTest extends Component {
  ...
  async componentDidMount() {
    ...
    try {
      ...
      if (typeof navigator !== 'undefined' && navigator.product === 'ReactNative') {
        assertStrictEqual(await Linking.canOpenURL('https://github.com/hogusea/'), true, 'Linking can not open https url');
      } else {
        // skipping RN-specific test'
      }
  ...
  return (
    <SettingsScrollView automaticallyAdjustContentInsets contentInsetAdjustmentBehavior="automatic">
      <SettingsCard>
        ...
        <BlueSpacing20 />
        <SaveFileButton fileName="ishmael-selftest.txt" fileContent={'Success on ' + new Date().toUTCString()}>
