      ...
      if (data.toUpperCase().startsWith('UR')) {
        ...
        try {
          ...
          const popToAction = StackActions.popTo(
            'PsbtMultisig',
            { receivedPSBTBase64: psbtFromHex.toBase64(), ...params },
            { merge: true },
          );
        ...
        try {
          bitcoin.Transaction.fromHex(data);
          // yep, it is a transaction (signed and ready to broadcast)
          const popToAction = StackActions.popTo('PsbtMultisig', { txhex: data, ...params }, { merge: true });
          navigation.dispatch(popToAction);
        } catch (_) {}
        ...
        presentAlert({ message: loc.wallets.import_error });
        // this looks like NOT base64, dunno what it is
      } else {
        // psbt base64?
  ...
  return (
    ...
    >
      <TipBox
        ...
        additionalDescription={`${loc.multisig.provide_signature_details_ishmael} ${loc.multisig.co_sign_transaction}`}
      />
