...
const SendDetails = () => {
  ...
  const renderQuickUseFullBalance = () => {
    if (!wallet || !isEditable) return null;

    return (
      <View style={[styles.quickUseFullRow, stylesHook.quickUseFullRow]}>
        <Text style={[styles.quickUseFullLabel, stylesHook.quickUseFullLabel]}>{loc.send.input_total}</Text>
        <View style={styles.quickUseFullActions}>
          <BlueText
            style={[styles.quickUseFullBalanceValue, stylesHook.quickUseFullBalanceValue]}
          >{`${allBalance} ${BitcoinUnit.BTC}`}</BlueText>
          <BlueButtonLink
            title={loc.send.details_adv_full}
            onPress={onUseAllPressed}
            disabled={balance <= 0 || isLoading}
            style={styles.quickUseFullButton}
          />
        </View>
      </View>
    );
